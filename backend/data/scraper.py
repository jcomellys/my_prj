...
class RealtimeDataSimulator:
    ...
    def get_generation_data(self):
        ...
        # Base capacities - Complete SIN generation fleet (~5,045 MW)
        hf = conditions["hydro_factor"]
        wf = conditions["wind_factor"]
        sf = conditions["solar_factor"]
        plants = [
            # === HYDRO (1,848 MW installed) ===
            {"name": "C.H. Fortuna", "fuel": "Hydro", "capacity": 300,
             "factor": hf * random.uniform(0.7, 0.95)},
            {"name": "C.H. Bayano", "fuel": "Hydro", "capacity": 260,
             "factor": hf * random.uniform(0.5, 0.9)},
            {"name": "C.H. Changuinola I", "fuel": "Hydro", "capacity": 221,
             "factor": hf * random.uniform(0.6, 0.95)},
            {"name": "C.H. Estí", "fuel": "Hydro", "capacity": 120,
             "factor": hf * random.uniform(0.5, 0.9)},
            {"name": "C.H. El Alto", "fuel": "Hydro", "capacity": 72,
             "factor": hf * random.uniform(0.5, 0.9)},
            {"name": "C.H. Caldera", "fuel": "Hydro", "capacity": 60,
             "factor": hf * random.uniform(0.4, 0.85)},
            {"name": "C.H. Bajo Mina", "fuel": "Hydro", "capacity": 57,
             "factor": hf * random.uniform(0.5, 0.9)},
            {"name": "C.H. Los Valles", "fuel": "Hydro", "capacity": 54,
             "factor": hf * random.uniform(0.4, 0.85)},
            {"name": "C.H. Monte Lirio", "fuel": "Hydro", "capacity": 52,
             "factor": hf * random.uniform(0.5, 0.9)},
            {"name": "C.H. La Estrella", "fuel": "Hydro", "capacity": 48,
             "factor": hf * random.uniform(0.5, 0.85)},
            {"name": "C.H. ACP (Gatún+Madden)", "fuel": "Hydro", "capacity": 72,
             "factor": hf * random.uniform(0.4, 0.8)},
            {"name": "C.H. Lorena", "fuel": "Hydro", "capacity": 34,
             "factor": hf * random.uniform(0.4, 0.85)},
            {"name": "C.H. Pando", "fuel": "Hydro", "capacity": 33,
             "factor": hf * random.uniform(0.4, 0.85)},
            {"name": "C.H. Gualaca", "fuel": "Hydro", "capacity": 32,
             "factor": hf * random.uniform(0.4, 0.85)},
            {"name": "C.H. Bonyic", "fuel": "Hydro", "capacity": 32,
             "factor": hf * random.uniform(0.5, 0.9)},
            {"name": "C.H. Pequeñas (Agr.)", "fuel": "Hydro", "capacity": 149,
             "factor": hf * random.uniform(0.3, 0.75)},
            # === THERMAL - Gas Natural (603 MW) ===
            {"name": "C.T. Costa Norte (GNL)", "fuel": "Natural Gas", "capacity": 381,
             "factor": random.uniform(0.6, 0.92)},
            {"name": "C.T. Termo Colón (GENA)", "fuel": "Natural Gas", "capacity": 150,
             "factor": random.uniform(0.5, 0.88)},
            {"name": "C.T. Generadora Gatún", "fuel": "Natural Gas", "capacity": 72,
             "factor": random.uniform(0.3, 0.8)},
            # === THERMAL - Carbon (120 MW) ===
            {"name": "C.T. BLM Carbón", "fuel": "Coal/Pet Coke", "capacity": 120,
             "factor": random.uniform(0.5, 0.85)},
            # === THERMAL - Bunker/Diesel (822 MW) ===
            {"name": "C.T. ACP Térmicas", "fuel": "Diesel/Bunker", "capacity": 232,
             "factor": random.uniform(0.3, 0.65)},
            {"name": "C.T. BLM Cativá (MCI)", "fuel": "Diesel/Bunker", "capacity": 87,
             "factor": random.uniform(0.4, 0.75)},
            {"name": "C.T. Pan-Am", "fuel": "Diesel/Bunker", "capacity": 60,
             "factor": random.uniform(0.3, 0.7)},
            {"name": "C.T. Pacora/Pedregal", "fuel": "Diesel/Bunker", "capacity": 55,
             "factor": random.uniform(0.3, 0.8)},
            {"name": "C.T. Térmica Caribe", "fuel": "Diesel/Bunker", "capacity": 48,
             "factor": random.uniform(0.3, 0.7)},
            {"name": "C.T. Otras Térmicas", "fuel": "Diesel/Bunker", "capacity": 340,
             "factor": random.uniform(0.15, 0.5)},
            # === WIND (336 MW) ===
            {"name": "P.E. Penonomé", "fuel": "Wind", "capacity": 215,
             "factor": wf},
            {"name": "P.E. Toabré", "fuel": "Wind", "capacity": 66,
             "factor": wf * random.uniform(0.85, 1.1)},
            {"name": "P.E. AES Eólica", "fuel": "Wind", "capacity": 55,
             "factor": wf * random.uniform(0.8, 1.1)},
            # === SOLAR (696 MW) ===
            {"name": "P.S. Penonomé Solar", "fuel": "Solar", "capacity": 120,
             "factor": sf},
            {"name": "P.S. Dist. Coclé/Herrera", "fuel": "Solar", "capacity": 130,
             "factor": sf * random.uniform(0.9, 1.05)},
            {"name": "P.S. Dist. Chiriquí", "fuel": "Solar", "capacity": 160,
             "factor": sf * random.uniform(0.85, 1.05)},
            {"name": "P.S. Dist. Panamá", "fuel": "Solar", "capacity": 196,
             "factor": sf * random.uniform(0.9, 1.05)},
            {"name": "P.S. Dist. Azuero/Colón", "fuel": "Solar", "capacity": 90,
             "factor": sf * random.uniform(0.85, 1.0)},
        ]
