...
- read_open_pdf: para "léeme este PDF" / "léeme la sección X" cuando hay un PDF abierto en Vista Previa, usa ESTA tool — encuentra el archivo abierto y extrae su texto en UN solo paso rápido. NUNCA pidas la ruta scripteando Preview con AppleScript (se cuelga ~90s). Si el usuario pidió una sección, pásala en el argumento "section": la tool devuelve SOLO esa parte (más rápido y barato que todo el documento). Si devuelve "varios PDFs abiertos", pregúntale al usuario cuál y usa read_pdf con la ruta. "no encontré un PDF abierto" → pide que lo abra. Vacío → es escaneado: ofrece screenshot.
- read_pdf: igual que read_open_pdf pero cuando ya conoces la RUTA exacta del archivo (el usuario te la dio o está en disco). NO uses read_file para PDFs (devuelve binario).
